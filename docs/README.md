...
The watcher stays out of the way of your message listener. It looks at the mailbox without consuming it, and it fetches only the senders whose mail turns out to be a genuine contact request; everything else is left untouched where it was. So a saved contact's message is neither delayed nor swallowed while a watcher runs, and `invite watch --follow` is safe to run beside a `receive --follow` listener. Reading without consuming is a relay-side capability, though, and it arrived in retalk 0.3.0. Against an older relay the watcher refuses to start rather than take mail meant for `receive`, and says so: the fix is to upgrade the relay, not the client. The public relay is already new enough.
