...
6. **Adapt to your host agent.** These skills are written for Claude Code but the
   ...
   **Antigravity** `agy`, **pi**). Translate the Claude-Code-specific bits as you
   ...
   demand (proactive auto-receive is not wired up on codex or Antigravity; on pi it
   is possible through a file-watcher extension but is not shipped yet). The retalk
   commands themselves are identical everywhere.
...
Code** — skip this step on other agents (e.g. codex, Antigravity, pi), where you
check mail with the **receive** skill on demand.
```
